...
class HRPPlugin < DomainPlugin
    ...
    if @@generate_memory_module == false
      ...
      regions.each{ |region|
        if ! region.is_root? then
          nsp = "#{region.get_namespace_path.get_global_name}"
      ...
      Cell.get_cell_list2.each { |cell|
        ...
        if ct.class == Celltype && check_celltype_list.include?( ct ) == false
          ...
          if regions_hrp.include?( Region.get_root ) == true || regions_hrp.length > 1
            ...
            file = AppFile.open( "#{$gen}/tecsgen_#{regions_hrp[0].get_namespace_path.get_global_name}.cfg" )
          end
  ...
  def self.add_inter_domain_join_set join
    ...
    dbgPrint "--------- add_inter_domain:#{join.get_owner.get_name} => #{join.get_cell.get_name}-----\n"
  ...
  def self.get_inter_domain_join_roots rhs_cell
    dbgPrint "--------- get_inter_domain #{rhs_cell.get_name} -----\n"
    domain_roots = []
    set = get_inter_domain_join_set rhs_cell
    set.each{ |join|
      dbgPrint "--------- get_inter_domain:#{join.get_owner.get_name} => #{join.get_cell.get_name}-----\n"
    ...
    if rhs_cell.get_region.get_domain_root.get_domain_type.get_option == "user" then
      domain_roots << rhs_cell.get_region.get_domain_root
    end
