 ...
 *  の責任を負わない．
 * 
 *  $Id: test_calsvc.c 153 2018-12-17 02:02:45Z ertl-hiro $
 */
 ...
 *	4:	mget_lod(PRC1_DOM1, HIGH_PRIORITY, &load1)			... (D-U)
 *		assert(load1 == 1)
 *		mget_lod(PRC1_KERNEL, MID_PRIORITY, &load2)			... (D-U)
 *		assert(load2 == 1)
 ...
 *		ref_flg(FLG2, &pk_rflg)
 *		assert(pk_rflg.flgptn == 0xaaU)
 *	19:	mget_lod(PRC1_DOM1, LOW_PRIORITY, &load1)			... (D-S)
 *		assert(load1 == 1)
 *		mget_lod(PRC1_KERNEL, MID_PRIORITY, &load2)			... (D-S)
 *		assert(load2 == 1)
...
#define PRC1_KERNEL		SCHEDNO(PRC1, TDOM_KERNEL)
#define PRC1_DOM1		SCHEDNO(PRC1, DOM1)
...
task1(intptr_t exinf)
{
	...
	check_point(4);
	ercd = mget_lod(PRC1_DOM1, HIGH_PRIORITY, &load1);
	check_ercd(ercd, E_OK);

	check_assert(load1 == 1);

	ercd = mget_lod(PRC1_KERNEL, MID_PRIORITY, &load2);
	check_ercd(ercd, E_OK);
...
task2(intptr_t exinf)
{
	...
	check_point(19);
	ercd = mget_lod(PRC1_DOM1, LOW_PRIORITY, &load1);
	check_ercd(ercd, E_OK);
