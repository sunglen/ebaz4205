 ...
 *  の責任を負わない．
 * 
 *  $Id: test_sysman2.c 425 2019-10-01 12:50:34Z ertl-hiro $
 */
 ...
 *
 *	== TASK1（優先度：高，DOM1）==
 *	1:	mrot_rdq(SCHEDNO(PRC1, DOM1), MID_PRIORITY)			... (D-1)
 *	2:	act_tsk(TASK3)
 *	//	高：TASK1，中：TASK3，高：TASK5
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), MID_PRIORITY)			... (D-2)
 *	//	高：TASK1，中：TASK3，高：TASK5
 *		mget_nth(SCHEDNO_PRC1_DOM1, MID_PRIORITY, 0U, &tskid)
 *		assert(tskid == TASK3)
 *	3:	act_tsk(TASK2)
 *	//	高：TASK1，中：TASK3→TASK2，高：TASK5
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY)	... (D-3)(F-3)
 *	//	高：TASK1，中：TASK2→TASK3，高：TASK5
 *		mget_nth(SCHEDNO_PRC1_DOM1, MID_PRIORITY, 0U, &tskid)
 *		assert(tskid == TASK2)
 *	4:	loc_cpu()
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), MID_PRIORITY) -> E_CTX		... (A-1)
 *		unl_cpu()
 *		mrot_rdq(SCHEDNO(PRC1, DOMID_SMALL), MID_PRIORITY) -> E_ID	... (A-2)
 *		mrot_rdq(SCHEDNO(PRC1, DOMID_LARGE), MID_PRIORITY) -> E_ID	... (A-3)
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SMALL_TASK) -> E_PAR		... (A-4)
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_LARGE) -> E_PAR			... (A-5)
 *		mrot_rdq(SCHEDNO(PRC1, DOM2), MID_PRIORITY) -> E_OACV		... (A-6)
 *		act_tsk(TASK4)
 *	//	高：TASK1，中：TASK2→TASK3→TASK4，高：TASK5
 *		slp_tsk()
 *	== TASK2（優先度：中，DOM1）==
 *	5:	mrot_rdq(SCHEDNO(PRC1, DOM1), MID_PRIORITY)			... (D-4)(D-5)
 *	//	中：TASK3→TASK4→TASK2，高：TASK5
 *	== TASK3（優先度：中，DOM1）==
 *	6:	mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SELF)			... (G-2)
 ...
 *	== ALM1 ==
 *	8:	loc_cpu()
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), MID_PRIORITY) -> E_CTX		... (B-1)
 *		unl_cpu()
 *		mrot_rdq(SCHEDNO(PRC1, DOMID_SMALL), MID_PRIORITY) -> E_ID	... (B-2)
 *		mrot_rdq(SCHEDNO(PRC1, DOMID_LARGE), MID_PRIORITY) -> E_ID	... (B-3)
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SMALL_INT) -> E_PAR		... (B-4)
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_LARGE) -> E_PAR			... (B-5)
 *		mrot_rdq(SCHEDNO(PRC1, DOM2), MID_PRIORITY)					... (B-6)
 *		mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SELF) -> E_PAR			... (G-1)
 *		SET(task4_flag)
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), MID_PRIORITY)		... (C-1)
 *		act_tsk(TASK9)
 *	//	中：TASK9⇒TASK4→TASK2→TASK3，高：TASK5
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), MID_PRIORITY)		... (C-2)
 *	//	中：TASK9⇒TASK4→TASK2→TASK3，高：TASK5
 *		act_tsk(TASK10)
 *	//	中：TASK9→TASK10⇒TASK4→TASK2→TASK3，高：TASK5
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), MID_PRIORITY)		... (C-3)
 ...
 *	== EXTSVC3 ==
 *	10:	mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY)		... (F-6)
 ...
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), MID_PRIORITY)		... (C-4)(C-5)
 *	//	中：TASK10→TASK11→TASK9⇒TASK4→TASK2→TASK3，高：TASK5
 *	== TASK10（優先度：中，カーネルドメイン）==
 *	13:	mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY)		... (F-2)
 *	//	中：TASK11→TASK9→TASK10⇒TASK4→TASK2→TASK3，高：TASK5
 *	== TASK11（優先度：中，カーネルドメイン）==
 *	14:	cal_svc(TFN_EXTSVC2, 0, 0, 0, 0, 0) 
 *	== EXTSVC2 ==
 *	15:	mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), TPRI_SELF)			... (F-5)(G-4)
 *	//	中：TASK9→TASK10→TASK11⇒TASK4→TASK2→TASK3，高：TASK5
 *	== TASK9（続き）==
 *	16:	sta_alm(ALM2, TEST_TIME_CP)
 *		WAIT(task9_flag)
 *	== ALM2 ==
 *	17:	mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY)		... (F-1)
 *	//	中：TASK10→TASK11→TASK9⇒TASK4→TASK2→TASK3，高：TASK5
 *		cal_svc(TFN_EXTSVC1, 0, 0, 0, 0, 0) 
 *	== EXTSVC1 ==
 *	18:	mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), TPRI_SELF) -> E_PAR	... (G-3)
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY)			... (F-4)
 *	//	中：TASK11→TASK9→TASK10⇒TASK4→TASK2→TASK3，高：TASK5
 *		RETURN(E_OK)
 *	== ALM2（続き）==
 *	19:	SET(task9_flag)
 ...
 *	//	高：TASK5
 *	== TASK5（優先度：高，DOM2）==
 *	27:	mrot_rdq(SCHEDNO(PRC1, DOM2), MID_PRIORITY)				... (E-1)
 *	28:	act_tsk(TASK7)
 *	//	高：TASK5，中：TASK7
 *		mrot_rdq(SCHEDNO(PRC1, DOM2), MID_PRIORITY)				... (E-2)
 *	//	高：TASK5，中：TASK7
 *		mget_nth(SCHEDNO_PRC1_DOM2, MID_PRIORITY, 0U, &tskid)
 *		assert(tskid == TASK7)
 *	29:	act_tsk(TASK6)
 *	//	高：TASK5，中：TASK7→TASK6
 *		mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY)		... (E-3)(F-3)
 ...
 *	//	中：TASK6→TASK7→TASK8
 *	== TASK6（優先度：中，DOM2）==
 *	30:	mrot_rdq(SCHEDNO(PRC1, DOM2), MID_PRIORITY)				... (E-4)(E-5)
 *	//	中：TASK7→TASK8→TASK6
 *	== TASK7（優先度：中，DOM2）==
 *	31:	ext_tsk()
 *	//	中：TASK8→TASK6
 ...
 *  mget_nthの第1パラメータ
 *
 *  以下の定義を用いないと，gentestが，返値を入れる変数名の取得に失敗
 *  する．
 */
#define SCHEDNO_PRC1_DOM1	SCHEDNO(PRC1, DOM1)
#define SCHEDNO_PRC1_DOM2	SCHEDNO(PRC1, DOM2)
...
alarm1_handler(intptr_t exinf)
{
	...
	ercd = mrot_rdq(SCHEDNO(PRC1, DOM1), MID_PRIORITY);
	check_ercd(ercd, E_CTX);

	ercd = unl_cpu();
	check_ercd(ercd, E_OK);

	ercd = mrot_rdq(SCHEDNO(PRC1, DOMID_SMALL), MID_PRIORITY);
	check_ercd(ercd, E_ID);

	ercd = mrot_rdq(SCHEDNO(PRC1, DOMID_LARGE), MID_PRIORITY);
	check_ercd(ercd, E_ID);

	ercd = mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SMALL_INT);
	check_ercd(ercd, E_PAR);

	ercd = mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_LARGE);
	check_ercd(ercd, E_PAR);

	ercd = mrot_rdq(SCHEDNO(PRC1, DOM2), MID_PRIORITY);
	check_ercd(ercd, E_OK);

	ercd = mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SELF);
	check_ercd(ercd, E_PAR);

	SET(task4_flag);

	ercd = mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), MID_PRIORITY);
	check_ercd(ercd, E_OK);
...
alarm2_handler(intptr_t exinf)
{
	...
	check_point(17);
	ercd = mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), MID_PRIORITY);
	check_ercd(ercd, E_OK);
								...
								intptr_t par4, intptr_t par5, ID cdmid)
{
	...
	check_point(18);
	ercd = mrot_rdq(SCHEDNO(PRC1, TDOM_KERNEL), TPRI_SELF);
	check_ercd(ercd, E_PAR);
								...
								intptr_t par4, intptr_t par5, ID cdmid)
{
	...
	check_point(15);
	ercd = mrot_rdq(SCHEDNO(PRC1, TDOM_SELF), TPRI_SELF);
	check_ercd(ercd, E_OK);
...
task1(intptr_t exinf)
{
	...
	ercd = mget_nth(SCHEDNO_PRC1_DOM1, MID_PRIORITY, 0U, &tskid);
	check_ercd(ercd, E_OK);
	...
	ercd = mrot_rdq(SCHEDNO(PRC1, DOM1), TPRI_SMALL_TASK);
	check_ercd(ercd, E_PAR);
...
task5(intptr_t exinf)
{
	...
	ercd = mget_nth(SCHEDNO_PRC1_DOM2, MID_PRIORITY, 0U, &tskid);
	check_ercd(ercd, E_OK);
