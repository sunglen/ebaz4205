 ...
 *  の責任を負わない．
 * 
 *  $Id: alarm.c 660 2019-12-15 04:14:19Z ertl-hiro $
 */
...
initialize_alarm(PCB *p_my_pcb)
{
	...
	for (i = 0; i < tnum_alm; i++) {
		if (alminib_table[i].iprcid == p_my_pcb->prcid) {
			...
			p_almcb->p_tmevt_heap = p_almcb->p_alminib->p_dominib
...
sta_alm(ID almid, RELTIM almtim)
{
	...
	CHECK_UNL_MYSTATE(&p_selftsk, &selfdom, &context);
	CHECK_ID(VALID_ALMID(almid));
	p_almcb = get_almcb(almid);
	CHECK_PAR(VALID_RELTIM(almtim));
	CHECK_ACPTN(p_almcb->p_alminib->acvct.acptn1, selfdom);

	lock_cpu();
	acquire_glock();
	if (p_almcb->almsta) {
		tmevtb_dequeue(&(p_almcb->tmevtb), p_almcb->p_tmevt_heap,
														p_almcb->p_pcb);
	}
	else {
		p_almcb->almsta = true;
	}
	tmevtb_enqueue_reltim(&(p_almcb->tmevtb), almtim, p_almcb->p_tmevt_heap,
															p_almcb->p_pcb);
	ercd = E_OK;
	release_glock();
	unlock_cpu();
...
msta_alm(ID almid, RELTIM almtim, ID prcid)
{
	...
	CHECK_ACPTN(p_almcb->p_alminib->acvct.acptn1, selfdom);
	p_tmevt_heap = p_almcb->p_alminib->p_dominib
							->p_tmevt_heap[INDEX_PRC(prcid)];
	CHECK_PAR(p_tmevt_heap != NULL);
	...
	p_almcb->p_tmevt_heap = p_tmevt_heap;
	tmevtb_enqueue_reltim(&(p_almcb->tmevtb), almtim, p_tmevt_heap,
															p_almcb->p_pcb);
	ercd = E_OK;
	release_glock();
	unlock_cpu();
...
stp_alm(ID almid)
{
	...
	CHECK_ID(VALID_ALMID(almid));
	p_almcb = get_almcb(almid);
	CHECK_ACPTN(p_almcb->p_alminib->acvct.acptn2, selfdom);
...
ref_alm(ID almid, T_RALM *pk_ralm)
{
	...
	CHECK_ID(VALID_ALMID(almid));
	CHECK_MACV_WRITE(pk_ralm, T_RALM, p_selftsk, selfdom);
	p_almcb = get_almcb(almid);
	CHECK_ACPTN(p_almcb->p_alminib->acvct.acptn4, selfdom);
