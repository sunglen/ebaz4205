 ...
 *  の責任を負わない．
 * 
 *  $Id: mem_manage.c 325 2019-03-12 19:47:32Z ertl-hiro $
 */
															...
															ACPTN selfdom)
{
	...
	else if (selfdom != TACP_KERNEL
				&& (selfdom & meminib_table[meminib].acptn4) == 0U) {
...
prb_mem(const void *base, size_t size, ID tskid, MODE pmmode)
{
	...
	CHECK_TSKCTX_MYSTATE(&p_selftsk, &selfdom);			/*［NGKI2925］*/
	CHECK_PAR(size > 0U);						/*［NGKI2929］*/
	if (tskid == TSK_SELF) {
		p_tcb = p_selftsk;						/*［NGKI2939］*/
	}
	else {
	...
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn4, selfdom);	/*［NGKI3426］*/

	ercd = probe_memory(base, size, p_tcb, pmmode, selfdom);
...
ref_mem(const void *addr, T_RMEM *pk_rmem)
{
	...
	CHECK_TSKCTX_MYSTATE(&p_selftsk, &selfdom);				/*［NGKI3955］*/
	CHECK_MACV_WRITE(pk_rmem, T_RMEM, p_selftsk, selfdom);	/*［NGKI3958］*/
