 ...
 *  の責任を負わない．
 * 
 *  $Id: dataqueue.c 679 2019-12-20 06:51:34Z ertl-honda $
 */
...
snd_dtq(ID dtqid, intptr_t data)
{
	...
	CHECK_ACPTN(p_dtqcb->p_dtqinib->acvct.acptn1, selfdom);
...
rcv_dtq(ID dtqid, intptr_t *p_data)
{
	...
	p_dtqcb = get_dtqcb(dtqid);
	CHECK_ACPTN(p_dtqcb->p_dtqinib->acvct.acptn2, selfdom);
...
ini_dtq(ID dtqid)
{
	...
	CHECK_ID(VALID_DTQID(dtqid));
	p_dtqcb = get_dtqcb(dtqid);
	CHECK_ACPTN(p_dtqcb->p_dtqinib->acvct.acptn3, selfdom);
...
ref_dtq(ID dtqid, T_RDTQ *pk_rdtq)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_DTQID(dtqid));
	CHECK_MACV_WRITE(pk_rdtq, T_RDTQ, p_selftsk, selfdom);
	p_dtqcb = get_dtqcb(dtqid);
	CHECK_ACPTN(p_dtqcb->p_dtqinib->acvct.acptn4, selfdom);
