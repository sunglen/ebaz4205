 ...
 *  の責任を負わない．
 * 
 *  $Id: semaphore.c 304 2019-03-02 09:45:25Z ertl-hiro $
 */
...
sig_sem(ID semid)
{
	...
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn1, selfdom);
...
wai_sem(ID semid)
{
	...
	CHECK_ID(VALID_SEMID(semid));
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn2, selfdom);
...
ini_sem(ID semid)
{
	...
	CHECK_ID(VALID_SEMID(semid));
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn3, selfdom);
...
ref_sem(ID semid, T_RSEM *pk_rsem)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_SEMID(semid));
	CHECK_MACV_WRITE(pk_rsem, T_RSEM, p_selftsk, selfdom);
	p_semcb = get_semcb(semid);
	CHECK_ACPTN(p_semcb->p_seminib->acvct.acptn4, selfdom);
