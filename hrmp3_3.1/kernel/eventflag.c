 ...
 *  の責任を負わない．
 * 
 *  $Id: eventflag.c 304 2019-03-02 09:45:25Z ertl-hiro $
 */
...
set_flg(ID flgid, FLGPTN setptn)
{
	...
	CHECK_ACPTN(p_flgcb->p_flginib->acvct.acptn1, selfdom);
...
wai_flg(ID flgid, FLGPTN waiptn, MODE wfmode, FLGPTN *p_flgptn)
{
	...
	CHECK_MACV_WRITE(p_flgptn, FLGPTN, p_selftsk, selfdom);
	p_flgcb = get_flgcb(flgid);
	CHECK_ACPTN(p_flgcb->p_flginib->acvct.acptn2, selfdom);
...
ini_flg(ID flgid)
{
	...
	CHECK_ID(VALID_FLGID(flgid));
	p_flgcb = get_flgcb(flgid);
	CHECK_ACPTN(p_flgcb->p_flginib->acvct.acptn3, selfdom);
...
ref_flg(ID flgid, T_RFLG *pk_rflg)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_FLGID(flgid));
	CHECK_MACV_WRITE(pk_rflg, T_RFLG, p_selftsk, selfdom);
	p_flgcb = get_flgcb(flgid);
	CHECK_ACPTN(p_flgcb->p_flginib->acvct.acptn4, selfdom);
