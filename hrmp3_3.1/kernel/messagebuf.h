 ...
 *  の責任を負わない．
 * 
 *  $Id: messagebuf.h 447 2019-10-05 01:29:22Z ertl-hiro $
 */
...
/*
 *  メッセージバッファ管理ブロックへのポインタテーブル（kernel_cfg.c）
 */
extern MBFCB *const		p_mbfcb_table[];

/*
 *  メッセージバッファ管理ブロックからメッセージバッファIDを取り出すた
 *  めのマクロ
 */
#define	MBFID(p_mbfcb)	((ID)(((p_mbfcb)->p_mbfinib - mbfinib_table) \
															+ TMIN_MBFID))

/*
 *  メッセージバッファ機能の初期化
 */
extern void	initialize_messagebuf(PCB *p_my_pcb);
 ...
 *  メッセージバッファへのメッセージ送信
 */
extern bool_t	send_message(PCB *p_my_pcb, MBFCB *p_mbfcb,
										const void *msg, uint_t msgsz);

/*
 *  メッセージバッファ送信待ちタスクのチェック
 */
extern void	messagebuf_signal(PCB *p_my_pcb, MBFCB *p_mbfcb);

/*
 *  メッセージバッファからのメッセージ受信
 */
extern uint_t	receive_message(PCB *p_my_pcb, MBFCB *p_mbfcb, void *msg);

/*
 *  メッセージバッファ送信待ちタスクの待ち解除時処理
 */
extern void	messagebuf_dequeue_wobj(PCB *p_my_pcb, TCB *p_tcb);
