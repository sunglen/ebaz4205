 ...
 *  の責任を負わない．
 * 
 *  $Id: memory.c 325 2019-03-12 19:47:32Z ertl-hiro $
 */
...
void
initialize_memory(PCB *p_my_pcb)
{
}
...
bool_t
probe_mem_write(const void *base, size_t size, TCB *p_selftsk, ACPTN selfdom)
{
	int_t	meminib;
	ATR		accatr;
	size_t	memsize;
	...
	if (accatr == TA_NOEXS) {
		...
		return(within_ustack(base, size, p_selftsk));
	}
		 ...
		 */
		return((selfdom & meminib_table[meminib].acptn1) != 0U);
	}
}
...
bool_t
probe_mem_read(const void *base, size_t size, TCB *p_selftsk, ACPTN selfdom)
{
	int_t	meminib;
	ATR		accatr;
	size_t	memsize;
	...
	if (accatr == TA_NOEXS) {
		...
		return((accatr & TA_NOREAD) == 0U
					&& (selfdom & meminib_table[meminib].acptn2) != 0U);
	}
}
