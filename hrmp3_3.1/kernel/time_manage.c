 ...
 *  の責任を負わない．
 * 
 *  $Id: time_manage.c 726 2020-01-01 23:53:44Z ertl-hiro $
 */
...
set_tim(SYSTIM systim)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI3564］［NGKI3565］*/
	CHECK_ACPTN(sysstat_acvct.acptn3, selfdom);		/*［NGKI3566］*/
...
get_tim(SYSTIM *p_systim)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI2350］［NGKI2351］*/
	CHECK_MACV_WRITE(p_systim, SYSTIM, p_selftsk, selfdom);	/*［NGKI2353］*/
	CHECK_ACPTN(sysstat_acvct.acptn4, selfdom);		/*［NGKI2352］*/
...
adj_tim(int32_t adjtim)
{
	...
	CHECK_UNL_MYSTATE(&p_selftsk, &selfdom, &context);	/*［NGKI3583］*/
	CHECK_PAR(TMIN_ADJTIM <= adjtim && adjtim <= TMAX_ADJTIM);
												/*［NGKI3584］*/
	CHECK_ACPTN(sysstat_acvct.acptn3, selfdom);	/*［NGKI3585］*/
	...
	if (check_adjtim(adjtim, p_my_pcb)) {		/*［ASPD1052］*/
		...
		for (prcid = TMIN_PRCID; prcid <= TMAX_PRCID; prcid++) {
			p_pcb = get_pcb(prcid);
			if (p_pcb->p_tmevt_heap_kernel != NULL
									&& !(p_pcb->in_signal_time)) {
