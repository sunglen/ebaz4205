 ...
 *  の責任を負わない．
 * 
 *  $Id: mempfix.c 304 2019-03-02 09:45:25Z ertl-hiro $
 */
...
get_mpf(ID mpfid, void **p_blk)
{
	...
	CHECK_MACV_WRITE(p_blk, void *, p_selftsk, selfdom);
	p_mpfcb = get_mpfcb(mpfid);
	CHECK_ACPTN(p_mpfcb->p_mpfinib->acvct.acptn1, selfdom);
...
rel_mpf(ID mpfid, void *blk)
{
	...
	CHECK_ID(VALID_MPFID(mpfid));
	p_mpfcb = get_mpfcb(mpfid);
	CHECK_ACPTN(p_mpfcb->p_mpfinib->acvct.acptn2, selfdom);
...
ini_mpf(ID mpfid)
{
	...
	CHECK_ID(VALID_MPFID(mpfid));
	p_mpfcb = get_mpfcb(mpfid);
	CHECK_ACPTN(p_mpfcb->p_mpfinib->acvct.acptn3, selfdom);
...
ref_mpf(ID mpfid, T_RMPF *pk_rmpf)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_MPFID(mpfid));
	CHECK_MACV_WRITE(pk_rmpf, T_RMPF, p_selftsk, selfdom);
	p_mpfcb = get_mpfcb(mpfid);
	CHECK_ACPTN(p_mpfcb->p_mpfinib->acvct.acptn4, selfdom);
