 ...
 *  の責任を負わない．
 * 
 *  $Id: mutex.c 379 2019-06-08 05:54:29Z ertl-hiro $
 */
...
loc_mtx(ID mtxid)
{
	...
	CHECK_ACPTN(p_mtxcb->p_mtxinib->acvct.acptn1, selfdom);
...
ini_mtx(ID mtxid)
{
	...
	CHECK_ID(VALID_MTXID(mtxid));
	p_mtxcb = get_mtxcb(mtxid);
	CHECK_ACPTN(p_mtxcb->p_mtxinib->acvct.acptn3, selfdom);
...
ref_mtx(ID mtxid, T_RMTX *pk_rmtx)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_MTXID(mtxid));
	CHECK_MACV_WRITE(pk_rmtx, T_RMTX, p_selftsk, selfdom);
	p_mtxcb = get_mtxcb(mtxid);
	CHECK_ACPTN(p_mtxcb->p_mtxinib->acvct.acptn4, selfdom);
