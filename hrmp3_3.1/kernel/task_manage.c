 ...
 *  の責任を負わない．
 * 
 *  $Id: task_manage.c 745 2020-01-15 00:36:40Z ertl-hiro $
 */
...
act_tsk(ID tskid)
{
	...
	CHECK_UNL_MYSTATE(&p_selftsk, &selfdom, &context);	/*［NGKI1114］*/
	...
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn1, selfdom);	/*［NGKI1117］*/
...
mact_tsk(ID tskid, ID prcid)
{
	...
	PCB		*p_my_pcb;
	PCB		*p_new_pcb;
	SCHEDCB	*p_new_schedcb;

	LOG_MACT_TSK_ENTER(tskid, prcid);
	CHECK_UNL_MYSTATE(&p_selftsk, &selfdom, &context);	/*［NGKI1126］*/
	if (tskid == TSK_SELF && !context) {
	...
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn1, selfdom);		/*［NGKI1131］*/
	p_new_schedcb = p_tcb->p_dominib->p_schedcb[INDEX_PRC(prcid)];
	CHECK_PAR(p_new_schedcb != NULL);			/*［NGKI5109］*/
...
can_act(ID tskid)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1139］［NGKI1140］*/
	if (tskid == TSK_SELF) {
	...
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn1, selfdom);	/*［NGKI1143］*/
...
mig_tsk(ID tskid, ID prcid)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1148］［NGKI1149］*/
	if (tskid == TSK_SELF) {
	...
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn1, selfdom);		/*［NGKI1154］*/
	p_new_schedcb = p_tcb->p_dominib->p_schedcb[INDEX_PRC(prcid)];
	CHECK_PAR(p_new_schedcb != NULL);				/*［NGKI5110］*/
	...
	if (p_tcb->p_pcb != p_my_pcb) {
		...
		/* タイムイベントキューから削除 */
		tmevtb_dequeue(&(p_tcb->winfo.tmevtb),
					p_tcb->p_dominib->p_tmevt_heap[INDEX_PRC(p_my_pcb->prcid)],
					p_my_pcb);
		LOG_TSKMIG(p_tcb, p_my_pcb->prcid, prcid);
		p_tcb->p_pcb = p_new_pcb;
		p_tcb->p_schedcb = p_new_schedcb;
		/* マイグレート先のプロセッサのタイムイベントキューに挿入 */
		tmevtb_enqueue(&(p_tcb->winfo.tmevtb),
					p_tcb->p_dominib->p_tmevt_heap[INDEX_PRC(prcid)],
					p_new_pcb);
		ercd = E_OK;
	}
	release_glock();
...
get_tst(ID tskid, STAT *p_tskstat)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI3614］［NGKI3615］*/
	if (tskid == TSK_SELF) {
	...
	CHECK_MACV_WRITE(p_tskstat, STAT, p_selftsk, selfdom);	/*［NGKI3619］*/
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn4, selfdom);	/*［NGKI3618］*/
...
chg_pri(ID tskid, PRI tskpri)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1184］［NGKI1185］*/
	if (tskid == TSK_SELF) {
	...
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn2, selfdom);	/*［NGKI1190］*/
	if (tskpri == TPRI_INI) {
...
get_pri(ID tskid, PRI *p_tskpri)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1203］［NGKI1204］*/
	if (tskid == TSK_SELF) {
	...
	CHECK_MACV_WRITE(p_tskpri, PRI, p_selftsk, selfdom);/*［NGKI1208］*/
	CHECK_ACPTN(p_tcb->p_tinib->acvct.acptn4, selfdom);	/*［NGKI1207］*/
...
get_inf(intptr_t *p_exinf)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);	/*［NGKI1213］［NGKI1214］*/
	CHECK_MACV_WRITE(p_exinf, intptr_t, p_selftsk, selfdom);	/*［NGKI1215］*/
