 ...
 *  の責任を負わない．
 * 
 *  $Id: spin_lock.c 41 2018-07-31 06:37:34Z ertl-honda $
 */
...
loc_spn(ID spnid)
{
	...
	CHECK_ACPTN(p_spninib->acvct.acptn1, selfdom);
...
unl_spn(ID spnid)
{
	...
	CHECK_ID(VALID_SPNID(spnid));
	p_spninib = get_spninib(spnid);
	CHECK_ACPTN(p_spninib->acvct.acptn1, get_mydom());
...
ref_spn(ID spnid, T_RSPN *pk_rspn)
{
	...
	p_spninib = get_spninib(spnid);
	CHECK_ACPTN(p_spninib->acvct.acptn4, selfdom);			/*［NGKI2211］*/
	CHECK_MACV_WRITE(pk_rspn, T_RSPN, p_selftsk, selfdom);	/*［NGKI2212］*/
