 ...
 *  の責任を負わない．
 * 
 *  $Id: pridataq.c 405 2019-07-25 15:37:57Z ertl-hiro $
 */
...
snd_pdq(ID pdqid, intptr_t data, PRI datapri)
{
	...
	CHECK_ACPTN(p_pdqcb->p_pdqinib->acvct.acptn1, selfdom);
...
rcv_pdq(ID pdqid, intptr_t *p_data, PRI *p_datapri)
{
	...
	CHECK_DISPATCH_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_PDQID(pdqid));
	CHECK_MACV_WRITE(p_data, intptr_t, p_selftsk, selfdom);
	CHECK_MACV_WRITE(p_datapri, PRI, p_selftsk, selfdom);
	p_pdqcb = get_pdqcb(pdqid);
	CHECK_ACPTN(p_pdqcb->p_pdqinib->acvct.acptn2, selfdom);
...
ini_pdq(ID pdqid)
{
	...
	CHECK_ID(VALID_PDQID(pdqid));
	p_pdqcb = get_pdqcb(pdqid);
	CHECK_ACPTN(p_pdqcb->p_pdqinib->acvct.acptn3, selfdom);
...
ref_pdq(ID pdqid, T_RPDQ *pk_rpdq)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_PDQID(pdqid));
	CHECK_MACV_WRITE(pk_rpdq, T_RPDQ, p_selftsk, selfdom);
	p_pdqcb = get_pdqcb(pdqid);
	CHECK_ACPTN(p_pdqcb->p_pdqinib->acvct.acptn4, selfdom);
