 ...
 *  の責任を負わない．
 * 
 *  $Id: cyclic.c 660 2019-12-15 04:14:19Z ertl-hiro $
 */
...
initialize_cyclic(PCB *p_my_pcb)
{
	...
	for (i = 0; i < tnum_cyc; i++) {
		if(cycinib_table[i].iprcid == p_my_pcb->prcid) {
			...
			p_cyccb->p_tmevt_heap = p_cyccb->p_cycinib->p_dominib
			...
			if ((p_cyccb->p_cycinib->cycatr & TA_STA) != 0U) {
				...
				tmevtb_register(&(p_cyccb->tmevtb), p_cyccb->p_tmevt_heap);
			}
			else {
...
sta_cyc(ID cycid)
{
	...
	CHECK_ACPTN(p_cyccb->p_cycinib->acvct.acptn1, selfdom);

	lock_cpu();
	acquire_glock();
	if (p_cyccb->cycsta) {
		tmevtb_dequeue(&(p_cyccb->tmevtb), p_cyccb->p_tmevt_heap,
														p_cyccb->p_pcb);
	}
	else {
		p_cyccb->cycsta = true;
	...
	tmevtb_enqueue_reltim(&(p_cyccb->tmevtb), p_cyccb->p_cycinib->cycphs,
									p_cyccb->p_tmevt_heap, p_cyccb->p_pcb);
	ercd = E_OK;
	release_glock();
	unlock_cpu();
...
msta_cyc(ID cycid, ID prcid)
{
	...
	CHECK_ACPTN(p_cyccb->p_cycinib->acvct.acptn1, selfdom);
	p_tmevt_heap = p_cyccb->p_cycinib->p_dominib
	...
	p_cyccb->p_tmevt_heap = p_tmevt_heap;
	/*
	 *  初回の起動のためのタイムイベントを登録する［ASPD1036］．
	 */
	tmevtb_enqueue_reltim(&(p_cyccb->tmevtb), p_cyccb->p_cycinib->cycphs,
												p_tmevt_heap, p_cyccb->p_pcb);
	ercd = E_OK;
	release_glock();
	unlock_cpu();
...
stp_cyc(ID cycid)
{
	...
	CHECK_ID(VALID_CYCID(cycid));
	p_cyccb = get_cyccb(cycid);
	CHECK_ACPTN(p_cyccb->p_cycinib->acvct.acptn2, selfdom);
...
ref_cyc(ID cycid, T_RCYC *pk_rcyc)
{
	...
	CHECK_TSKCTX_UNL_MYSTATE(&p_selftsk, &selfdom);
	CHECK_ID(VALID_CYCID(cycid));
	CHECK_MACV_WRITE(pk_rcyc, T_RCYC, p_selftsk, selfdom);
	p_cyccb = get_cyccb(cycid);
	CHECK_ACPTN(p_cyccb->p_cycinib->acvct.acptn4, selfdom);
