 ...
 *  の責任を負わない．
 * 
 *  $Id: gic_kernel_impl.h 330 2019-03-18 10:18:49Z ertl-hiro $
 */
 ...
 *  CPUインタフェースの初期化
 */
extern void gicc_initialize(PCB *p_my_pcb);
...
{
	data_sync_barrier();
	sil_swrw_mem(GICD_SGIR,
			(1U << (prcid - 1 + GICD_SGIR_CPUTARGET_OFFSET)) | intno);
}
