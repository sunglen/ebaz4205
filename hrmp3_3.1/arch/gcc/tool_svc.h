...
{
	CAL_SVC_2M(ER, TFN_REF_SPN, ID, spnid, T_RSPN *, pk_rspn);
}

Inline ER
mact_tsk(ID tskid, ID prcid)
{
	CAL_SVC_2M(ER, TFN_MACT_TSK, ID, tskid, ID, prcid);
}

Inline ER
mig_tsk(ID tskid, ID prcid)
{
	CAL_SVC_2M(ER, TFN_MIG_TSK, ID, tskid, ID, prcid);
}
...
{
	CAL_SVC_2M(ER, TFN_MSTA_CYC, ID, cycid, ID, prcid);
}

Inline ER
msta_alm(ID almid, RELTIM almtim, ID prcid)
{
	CAL_SVC_3M(ER, TFN_MSTA_ALM, ID, almid, RELTIM, almtim, ID, prcid);
}
...
get_pid(ID *p_prcid)
{
	CAL_SVC_1M(ER, TFN_GET_PID, ID *, p_prcid);
}
...
loc_spn(ID spnid)
{
	CAL_SVC_1M(ER, TFN_LOC_SPN, ID, spnid);
}

Inline ER
try_spn(ID spnid)
{
	CAL_SVC_1M(ER, TFN_TRY_SPN, ID, spnid);
}

Inline ER
unl_spn(ID spnid)
{
	CAL_SVC_1M(ER, TFN_UNL_SPN, ID, spnid);
}
