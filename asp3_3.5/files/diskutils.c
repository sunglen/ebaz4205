...
#define NUM_VOLUME_CMD      7

static int_t volume_format(int argc, char **argv);
static int_t volume_dir(int argc, char **argv);
static int_t volume_cat(int argc, char **argv);
static int_t volume_mktestfile(int argc, char **argv);
...
static const COMMAND_INFO volume_command_info[] = {
	...
	{"CAT",			volume_cat    },	/* ファイル表示 */
	{"MKTESTFILE",	volume_mktestfile  },	/* ファイル作成 */
...
volume_format(int argc, char **argv)
{
	...
	if(argc > 1){
		...
		if(psdev->pdevf->_sdevf_sense == 0 || psdev->pdevf->_sdevf_diskioctl == 0){
			printf("\nno function !\n");
			return -1;
		}
		...
		//get sector size
		res=psdev->pdevf->_sdevf_diskioctl(psdev, GET_SECTOR_SIZE, &sector_size);
		...
		printf(" format[%d] with %d bytes sector size ...\n", devno, sector_size);

		//if set allocsize=0(auto), f_mkfs will fail.
		if((res=f_mkfs(devno, 0, sector_size)) == FR_OK)
			printf("   Ok\n");
		else
			printf("   Error %d\n", res);
	}
...
volume_cat(int argc, char **argv)
{
	...
	if(argc > 1){
		...
		if(psdev->pdevff == 0 || psdev->pdevff->_sdevff_lseek == 0 || psdev->pdevff->_sdevff_read == 0){
			printf("\nno function !\n");
			return -1;
		}
		printf(" cat[%s]\n", path);
		if((fd=psdev->pdevff->_sdevff_open(devno, spath, O_RDONLY)) >= 0){
			psdev->pdevff->_sdevff_lseek(fd, 0, SEEK_SET, &res);
			psdev->pdevff->_sdevff_read(fd, ReadBuffer, sizeof(ReadBuffer), &res);
			if(res == 0){
				printf("   Ok\n");
				printf("%s", ReadBuffer);
			}else{
...
static int_t
volume_mktestfile(int argc, char **argv)
{
	StorageDevice_t *psdev;
	char   *path, *spath;
	int    devno;
	int fd;
	int res;
	char WriteBuffer[]= "TOPPERS/ASP3 + TECS + FATFS TEST FILE\n";
	...
	if(argc > 1){
		...
		if(psdev->pdevff == 0 || psdev->pdevff->_sdevff_lseek == 0 || psdev->pdevff->_sdevff_write == 0){
			printf("\nno function !\n");
			return -1;
		}
		printf(" mktestfile[%s]\n", path);
		if((fd=psdev->pdevff->_sdevff_open(devno, spath, O_CREAT | O_WRONLY | O_TRUNC)) >= 0){
			//psdev->pdevff->_sdevff_lseek(fd, 0, SEEK_SET, &res);
			psdev->pdevff->_sdevff_write(fd, WriteBuffer, sizeof(WriteBuffer), &res);
			if(res == 0){
				printf("   Ok\n");
				//printf("%s\n", WriteBuffer);
			}else{
